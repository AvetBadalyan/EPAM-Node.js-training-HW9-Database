...
export const createNewMovie = async (title: string, releaseYear: number, directorID: number): Promise<Movie> => {
    return await movieModel.createMovie(title, releaseYear, directorID);
};

export const updateExistingMovie = async (id: number, title: string, releaseYear: number, directorID: number): Promise<Movie | null> => {
