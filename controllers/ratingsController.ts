...
export const getRatingByMovieId = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const rating = await ratingsService.fetchRatingByMovieId(
    ...
    if (rating) {
      ...
      res.status(404).json({
        message: `Rating with ID ${Number(req.params.movieId)} not found`,
      });
    }
  } catch (err) {
    const error = err as Error;
...
export const updateRating = async (
  ...
  try {
    ...
    const updatedRating = await ratingsService.updateExistingRating(
      Number(req.params.movieId),
      rating
    );
    if (updatedRating) {
